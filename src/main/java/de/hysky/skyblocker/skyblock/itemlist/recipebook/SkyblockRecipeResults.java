...
/**
 * Based off {@link RecipeBookResults}.
 */
public class SkyblockRecipeResults implements RecipeAreaDisplay {
	...
	private void drawRecipeDisplay(DrawContext context, TextRenderer textRenderer, int x, int y, int mouseX, int mouseY) {
		...
		if (this.hoveredText == null && mouseX >= x + 86 && mouseY >= y + 81 && mouseX < x + 86 + 25 && mouseY < y + 81 + 25 && recipe instanceof SkyblockKatUpgradeRecipe katUpgradeRecipe) {
			this.hoveredText = Text.of(katUpgradeRecipe.getUpgradeTimeString());
		}
	...
	private void updateResultButtons() {
		if (this.recipeView) {
			...
			switch (recipe) {
				...
				case SkyblockKatUpgradeRecipe katUpgradeRecipe -> {

					recipeIcon = new ItemStack(Items.LAVA_BUCKET);


					Vector2i gridSize = katUpgradeRecipe.getGridSize();
					// Using this slot as a center cuz I said so
					SkyblockRecipeResultButton button = this.resultButtons.get(11);
					int startX = button.getX() + button.getWidth() / 2 - (gridSize.x * 25) / 2;
					int startY = button.getY() + button.getHeight() / 2 - (gridSize.y * 25)/2;
					for (int i = 0; i < katUpgradeRecipe.getInputs().size(); i++) {
						int x = startX + (i % gridSize.x) * 25;
						int y = startY + (i / gridSize.x) * 25;
						recipeSlotButtons.add(new SkyblockRecipeResultButton(x, y).setDisplayStack(katUpgradeRecipe.getInputs().get(i)));
					}
					//Result
					recipeSlotButtons.add(this.resultButtons.get(14).setDisplayStack(katUpgradeRecipe.getResult()));
				}
				case null, default -> {}
			}
		} else {
